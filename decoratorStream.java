// 9. Decorator StreamJAVA  STREAM  PUBLIC  NEW
...
public class DecoratorStream extends OutputStream
{
    ...
    public void write(byte[] b, int off, int len) throws IOException {
        ...
        byte[] bytePrefix = prefix.getBytes(charsetName);
        
        byte[] prefixarr = new byte [bytePrefix.length+b.length];
    
        int index = 0;

        for(byte c : bytePrefix){
            prefixarr[index] = c;
            index++;
        }
        for(byte c : b){
